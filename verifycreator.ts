...
(async () => {
   ...
   */
  const creators: Creator[] = asset.creators.map((creator: { address: PublicKeyInitData; verified: any; share: any; }) => {
    return {
  ...
  let blockhash = await connection.getLatestBlockhash().then((res: { blockhash: any; }) => res.blockhash);
