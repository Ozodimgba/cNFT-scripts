...
import { CurrencyAmount, ONE, Percent, Price, Token, TokenAmount, ZERO } from '../entity';
...
export class TradeV2 extends Base {
    ...
    static getAllRouteComputeAmountOut({ inputTokenAmount, outputToken, directPath, routePathDict, simulateCache, tickCache, mintInfos, slippage, chainTime, epochInfo, feeConfig, }) {
        ...
        for (const itemPool of directPath) {
            if (itemPool.version === 6) {
                try {
                    const { realAmountIn, amountOut, minAmountOut, expirationTime, currentPrice, executionPrice, priceImpact, fee, remainingAccounts, } = Clmm.computeAmountOutFormat({
                        poolInfo: itemPool,
    ...
    static computeAmountOut({ middleMintInfo, amountIn, currencyOut, slippage, fromPool, toPool, simulateCache, tickCache, epochInfo, mintInfos, }) {
        ...
        if (fromPool.version === 6) {
            const { minAmountOut: _minMiddleAmountOut, priceImpact: _firstPriceImpact, fee: _firstFee, remainingAccounts: _firstRemainingAccounts, expirationTime: _expirationTime, realAmountIn: _realAmountIn, } = Clmm.computeAmountOutFormat({
                poolInfo: fromPool,
            ...
            const { minAmountOut: _minMiddleAmountOut, priceImpact: _firstPriceImpact, fee: _firstFee, } = Liquidity.computeAmountOut({
                poolKeys: jsonInfo2PoolKeys(fromPool),
        ...
        if (toPool.version === 6) {
            const { amountOut: _amountOut, minAmountOut: _minAmountOut, priceImpact: _secondPriceImpact, fee: _secondFee, remainingAccounts: _secondRemainingAccounts, expirationTime: _expirationTime, realAmountIn: _realAmountIn, } = Clmm.computeAmountOutFormat({
                poolInfo: toPool,
                tickArrayCache: tickCache[toPool.id.toString()],
                amountIn: new TokenAmount(minMiddleAmountOut.amount.token, minMiddleAmountOut.amount.raw.sub(minMiddleAmountOut.fee === undefined ? ZERO : minMiddleAmountOut.fee.raw)),
                currencyOut,
                slippage,
                token2022Infos: mintInfos,
                epochInfo,
            ...
            const { amountOut: _amountOut, minAmountOut: _minAmountOut, priceImpact: _secondPriceImpact, fee: _secondFee, } = Liquidity.computeAmountOut({
                poolKeys: jsonInfo2PoolKeys(toPool),
        ...
        return {
            minMiddleAmountFee: minMiddleAmountOut.fee !== undefined
                ? new TokenAmount(middleToken, (minMiddleAmountOut.fee?.raw ?? new BN(0)).add(realAmountRouteIn.fee?.raw ?? new BN(0)))
                : undefined,
            middleToken,
            realAmountIn,
            amountOut,
